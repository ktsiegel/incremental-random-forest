...
private[ml] object WahooRandomForest extends Logging {
  ...
  /**
   * Initial training of an online random forest.
  ...
  def run(
    ...
    println("algo = " + strategy.algo)
    println("numTrees = " + numTrees)
    println("seed = " + seed)
    println("maxBins = " + metadata.maxBins)
    println("featureSubsetStrategy = " + featureSubsetStrategy)
    println("numFeaturesPerNode = " + metadata.numFeaturesPerNode)
    println("subsamplingRate = " + strategy.subsamplingRate)

    // Find the splits and the corresponding bins (interval between the splits) using a sample
    // of the input data.
    timer.start("findSplitsBins")
    // Returns an array of length equal to the number of features. Each slot in the
    // array holds an array of candidate splits for that feature.
    // A Split object handles decisions of whether to sort left or sort right
    // in a node.
    val splits: Array[Array[Split]] = findSplits(retaggedInput, metadata, seed)
    ...
    // A TreePoint encapsulates a labeled point's label and an array
    // that, for each feature, contains the bin in which the point belongs.
    ...
    val nodeQueue = new mutable.Queue[(Int, WahooNode)]()

    val rng = new Random()
    rng.setSeed(seed)

    // Allocate and queue root nodes.
    val topNodes = Array.fill[WahooNode](numTrees)(new WahooNode(treeInput, rng, null))
