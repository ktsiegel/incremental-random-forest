...
private[ml] object WahooRandomForest extends Logging {
  ...
  /**
    ...
    */
  private[wahoo] def findBestSplits(
  ...
  /**
    ...
    * @return tuple for best split: (Split, information gain, prediction at node)
    */
  private def binsToBestSplitRandomized(
                               binAggregates: DTStatsAggregator,
                               splits: Array[Array[Split]],
                               featuresForNode: Option[Array[Int]],
                               node: LearningNode): (Split, ImpurityStats) = {
    ...
    val (bestSplit, bestSplitStats) =
      Range(0, binAggregates.metadata.numFeaturesPerNode).map { featureIndexIdx =>
        val featureIndex = if (featuresForNode.nonEmpty) {
          // Used if we are subsampling features
          featuresForNode.get.apply(featureIndexIdx)
        } else {
          featureIndexIdx
        }
        ...
        if (binAggregates.metadata.isContinuous(featureIndex)) {
          ...
          // Find random split.
          val randSplitIndex = r.nextInt(numSplits)
          val leftChildStats = binAggregates.getImpurityCalculator(nodeFeatureOffset, randSplitIndex)
          val rightChildStats =
            binAggregates.getImpurityCalculator(nodeFeatureOffset, numSplits)
          rightChildStats.subtract(leftChildStats)
          gainAndImpurityStats = calculateImpurityStats(gainAndImpurityStats,
            leftChildStats, rightChildStats, binAggregates.metadata)
          (splits(featureIndex)(randSplitIndex), gainAndImpurityStats)
        } else if (binAggregates.metadata.isUnordered(featureIndex)) {
          // Unordered categorical feature
          val (leftChildOffset, rightChildOffset) =
            binAggregates.getLeftRightFeatureOffsets(featureIndexIdx)
          val randSplitIndex = r.nextInt(numSplits)
          val leftChildStats = binAggregates.getImpurityCalculator(leftChildOffset, randSplitIndex)
          val rightChildStats =
            binAggregates.getImpurityCalculator(rightChildOffset, randSplitIndex)
          gainAndImpurityStats = calculateImpurityStats(gainAndImpurityStats,
            leftChildStats, rightChildStats, binAggregates.metadata)
          (splits(featureIndex)(randSplitIndex), gainAndImpurityStats)
        } else {
          ...
          // Find best split.
          val randSplitIndex = r.nextInt(numSplits)
          val featureValue = categoriesSortedByCentroid(randSplitIndex)._1
          val leftChildStats =
            binAggregates.getImpurityCalculator(nodeFeatureOffset, featureValue)
          val rightChildStats =
            binAggregates.getImpurityCalculator(nodeFeatureOffset, lastCategory)
          ...
          gainAndImpurityStats = calculateImpurityStats(gainAndImpurityStats,
            leftChildStats, rightChildStats, binAggregates.metadata)
          val categoriesForSplit =
            categoriesSortedByCentroid.map(_._1.toDouble).slice(0, randSplitIndex + 1)
          val randFeatureSplit =
            new CategoricalSplit(featureIndex, categoriesForSplit.toArray, numCategories)
          (randFeatureSplit, gainAndImpurityStats)
        }
      }.maxBy(_._2.gain)
  ...
  /**
    ...
    */
  protected[wahoo] def findSplits(
                                  input: RDD[LabeledPoint],
  ...
  /**
    ...
    */
  private[wahoo] def extractMultiClassCategories(
                                                 input: Int,
  ...
  /**
    ...
    */
  private[wahoo] def findSplitsForContinuousFeature(
  ...
  private[wahoo] class NodeIndexInfo(
  ...
  /**
    ...
    */
  private[wahoo] def selectNodesToSplit(
                                        nodeQueue: mutable.Queue[(Int, LearningNode)],
                                        maxMemoryUsage: Long,
                                        metadata: DecisionTreeMetadata,
    ...
    while (nodeQueue.nonEmpty && memUsage < maxMemoryUsage) {
      ...
      val nodeMemUsage = WahooRandomForest.aggregateSizeForNode(metadata, featureSubset) * 8L
  ...
  /**
    ...
    */
  private[wahoo] def computeFeatureImportance(
                                              node: Node,
  ...
  /**
    ...
    */
  private[wahoo] def normalizeMapValues(map: OpenHashMap[Int, Double]): Unit = {
