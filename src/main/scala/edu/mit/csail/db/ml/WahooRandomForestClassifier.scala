...
class WahooRandomForestClassifier(uid: String, wc: Option[WahooContext]) 
  ...
  def this(wc: Option[WahooContext]) = this(Identifiable.randomUID("wrfc"), wc)
   ...
   */
  override def trainMore(oldModel: RandomForestClassificationModel, dataset: DataFrame, addedTrees: Int): RandomForestClassificationModel = {
